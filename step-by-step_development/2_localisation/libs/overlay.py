...
class overlay_on():
    ...
    def put_overlay(self, frame):
        self.frame = frame

        self.load_marker_params()
        self.put_overlay_for_each_marker()
        self.draw_midline()

        return self.frame

    def put_overlay_for_each_marker(self):
        frame = self.frame
        ...
        if len(corners) > 0:
            ...
            for (markerCorners,markerID) in zip (corners, ids): #zip combines the corners and id, essentially creates a relationship
                ...
                self.draw_center(topLeft, topRight)
    ...
    def draw_center(self, topLeft, topRight):
        cX,cY = self.compute_marker_center(topLeft, topRight)
